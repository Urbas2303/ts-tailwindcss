const Footer = () => {
	return (
		<div className='bg-footer-texture bg-lime-950/95 w-full min-h-screen object-cover'>
			<div className='md:flex md:space-x-8 mx-2 sm:mx-32 my-10 pt-60 border-b-2 pb-10'>
				<div>
					<h6 className='text-lime-200'>Phone</h6>
					<a className='text-white' href='tel:+3250312832'>
						+32 50 31 28 32
					</a>
				</div>
				<div>
					<h6 className='text-lime-200'>Address</h6>
					<p className='text-white'>491 Merlin Crest Suite 963</p>
				</div>
			</div>
			<div className='md:flex mx-2 sm:mx-32 mt-10 mb-5 border-b-2 pb-5'>
				<div className='w-2/3 px-5 md:flex md:space-x-5'>
					<div>
						<h4 className='text-white font-semibold uppercase'>Services</h4>
						<a className='text-lime-200 block' href='#'>
							Strategy Design
						</a>
						<a className='text-lime-200 block' href='#'>
							Product Design
						</a>
						<a className='text-lime-200 block' href='#'>
							Content Strategy
						</a>
						<a className='text-lime-200 block' href='#'>
							Brand Strategy
						</a>
						<a className='text-lime-200 block' href='#'>
							Development
						</a>
					</div>
					<div>
						<h4 className='text-white font-semibold uppercase'>
							Help And Advice
						</h4>
						<a className='text-lime-200 block' href='#'>
							How it works
						</a>
						<a className='text-lime-200 block' href='#'>
							Contact Support
						</a>
						<a className='text-lime-200 block' href='#'>
							Privacy Policy
						</a>
						<a className='text-lime-200 block' href='#'>
							FAQ
						</a>
					</div>
					<div>
						<h4 className='text-white font-semibold uppercase'>Company</h4>
						<a className='text-lime-200 block' href='#'>
							About
						</a>
						<a className='text-lime-200 block' href='#'>
							Blog
						</a>
						<a className='text-lime-200 block' href='#'>
							Contact
						</a>
						<a className='text-lime-200 block' href='#'>
							Jobs
						</a>
					</div>
				</div>
				<div className='w-1/3 md:border-l-2 px-5'>
					<h4 className='text-white font-semibold uppercase'>Get in Touch</h4>
					<p className='text-lime-200 my-5'>
						Feel free to get in touch with us vai email
					</p>
					<h3 className='text-white text-2xl font-semibold'>
						info.webovio@gmail.com
					</h3>
					<div className='flex items-center justify-center space-x-2'>
						<svg
							fill='#d4d4d4'
							width='40px'
							height='40px'
							viewBox='0 0 24 24'
							xmlns='http://www.w3.org/2000/svg'
						>
							<path d='M12 2.03998C6.5 2.03998 2 6.52998 2 12.06C2 17.06 5.66 21.21 10.44 21.96V14.96H7.9V12.06H10.44V9.84998C10.44 7.33998 11.93 5.95998 14.22 5.95998C15.31 5.95998 16.45 6.14998 16.45 6.14998V8.61998H15.19C13.95 8.61998 13.56 9.38998 13.56 10.18V12.06H16.34L15.89 14.96H13.56V21.96C15.9164 21.5878 18.0622 20.3855 19.6099 18.57C21.1576 16.7546 22.0054 14.4456 22 12.06C22 6.52998 17.5 2.03998 12 2.03998Z' />
						</svg>
						<svg
							fill='#d4d4d4'
							version='1.1'
							id='Capa_1'
							xmlns='http://www.w3.org/2000/svg'
							xmlnsXlink='http://www.w3.org/1999/xlink'
							width='40px'
							height='40px'
							viewBox='0 0 97.75 97.75'
							xmlSpace='preserve'
						>
							<g>
								<path
									d='M48.875,0C21.882,0,0,21.882,0,48.875S21.882,97.75,48.875,97.75S97.75,75.868,97.75,48.875S75.868,0,48.875,0z
                                    M78.43,35.841c0.023,0.577,0.035,1.155,0.035,1.736c0,20.878-15.887,42.473-42.473,42.473c-8.127,0-16.04-2.319-22.883-6.708
                                    c-0.143-0.091-0.202-0.268-0.145-0.427c0.057-0.158,0.218-0.256,0.383-0.237c1.148,0.137,2.322,0.205,3.487,0.205
                                    c6.323,0,12.309-1.955,17.372-5.664c-6.069-0.512-11.285-4.619-13.161-10.478c-0.039-0.122-0.011-0.255,0.073-0.351
                                    c0.085-0.096,0.215-0.138,0.339-0.115c1.682,0.319,3.392,0.34,5.04,0.072c-6.259-1.945-10.658-7.808-10.658-14.483l0.002-0.194
                                    c0.003-0.127,0.072-0.243,0.182-0.306c0.109-0.064,0.245-0.065,0.355-0.003c1.632,0.906,3.438,1.488,5.291,1.711
                                    c-3.597-2.867-5.709-7.213-5.709-11.862c0-2.682,0.71-5.318,2.054-7.623c0.06-0.103,0.166-0.169,0.284-0.178
                                    c0.119-0.012,0.234,0.04,0.309,0.132c7.362,9.03,18.191,14.59,29.771,15.305c-0.193-0.972-0.291-1.974-0.291-2.985
                                    c0-8.361,6.802-15.162,15.162-15.162c4.11,0,8.082,1.689,10.929,4.641c3.209-0.654,6.266-1.834,9.09-3.508
                                    c0.129-0.077,0.291-0.065,0.41,0.028c0.116,0.094,0.164,0.25,0.118,0.394c-0.957,2.993-2.823,5.604-5.33,7.489
                                    c2.361-0.411,4.652-1.105,6.831-2.072c0.146-0.067,0.319-0.025,0.424,0.098c0.104,0.124,0.113,0.301,0.023,0.435
                                    C83.759,31.175,81.299,33.744,78.43,35.841z'
								/>
							</g>
						</svg>
						<svg
							fill='#d4d4d4'
							width='40px'
							height='40px'
							viewBox='0 0 24 24'
							xmlns='http://www.w3.org/2000/svg'
						>
							<path d='M22.014 11.974C21.996 6.462 17.521 2 12.008 2 6.493 2 2.019 6.462 2.001 11.974L2 11.948v.112l.001-.023c.017 5.513 4.491 9.977 10.007 9.977 5.514 0 9.988-4.462 10.006-9.974l.001.026v-.118l-.001.026zM9.281 16.557c-2.509 0-4.548-2.039-4.548-4.549s2.039-4.549 4.548-4.549c1.23 0 2.258.451 3.046 1.188l-1.295 1.255c-.325-.309-.899-.673-1.751-.673-1.505 0-2.733 1.251-2.733 2.785 0 1.533 1.229 2.784 2.733 2.784 1.742 0 2.384-1.206 2.502-1.92H9.279V11.18h4.255c.066.286.115.554.115.932 0 2.597-1.742 4.445-4.368 4.445zm10.458-4.095H17.92v1.819h-1.364v-1.819h-1.82v-1.364h1.82v-1.82h1.364v1.82h1.819v1.364z' />
						</svg>
						<svg
							fill='#d4d4d4'
							width='40px'
							height='40px'
							version='1.1'
							id='Layer_1'
							xmlns='http://www.w3.org/2000/svg'
							xmlnsXlink='http://www.w3.org/1999/xlink'
							viewBox='-143 145 512 512'
							xmlSpace='preserve'
						>
							<path
								d='M113,145c-141.4,0-256,114.6-256,256s114.6,256,256,256s256-114.6,256-256S254.4,145,113,145z M41.4,508.1H-8.5V348.4h49.9
                                    V508.1z M15.1,328.4h-0.4c-18.1,0-29.8-12.2-29.8-27.7c0-15.8,12.1-27.7,30.5-27.7c18.4,0,29.7,11.9,30.1,27.7
                                    C45.6,316.1,33.9,328.4,15.1,328.4z M241,508.1h-56.6v-82.6c0-21.6-8.8-36.4-28.3-36.4c-14.9,0-23.2,10-27,19.6
                                    c-1.4,3.4-1.2,8.2-1.2,13.1v86.3H71.8c0,0,0.7-146.4,0-159.7h56.1v25.1c3.3-11,21.2-26.6,49.8-26.6c35.5,0,63.3,23,63.3,72.4V508.1z
                                    '
							/>
						</svg>
					</div>
				</div>
			</div>
			<div className='md:flex mx-2 sm:mx-32 items-center justify-between'>
				<div className='md:flex'>
					<svg
						width='55'
						height='36'
						viewBox='0 0 55 36'
						fill='none'
						xmlns='http://www.w3.org/2000/svg'
					>
						<path
							fillRule='evenodd'
							clipRule='evenodd'
							d='M36.6502 30.4842C35.35 32.6512 32.2094 32.6511 30.9092 30.4842L12.7815 0.271275L48.8656 0.271275C51.4676 0.271275 53.0748 3.10989 51.7361 5.34111L36.6502 30.4842Z'
							fill='#FFD723'
						/>
						<path
							fillRule='evenodd'
							clipRule='evenodd'
							d='M18.1277 30.4842C19.4279 32.6512 22.5685 32.6511 23.8687 30.4842L41.9964 0.271275L5.91239 0.271275C3.31037 0.271275 1.70317 3.10989 3.0419 5.34111L18.1277 30.4842Z'
							fill='white'
						/>
						<path
							fillRule='evenodd'
							clipRule='evenodd'
							d='M12.7815 0.27124H41.9964L27.39 24.617L12.7815 0.27124Z'
							fill='#F6F6F6'
						/>
					</svg>
					<h2 className='ml-4 font-black uppercase text-xl text-white'>
						yukolokolcev.
					</h2>
				</div>
				<p className='text-lime-200 text-xs'>
					© 2020@webovio. All Rights Reserved.
				</p>
			</div>
		</div>
	)
}

export default Footer
